...
class TestCreateVisit:
    ...
    def test_create_visit_invalid_required_fields(
        self, api_manager, patient_context, visit_type_uuid: str, field_name: str, incorrect_value, expected_error: str
    ):
    ...
    @pytest.mark.check_visit_persisted(expected_count=1)
    def test_create_visit_with_another_active_visit(
        self, api_manager: ApiManager, create_visit_request: CreateVisitRequest
    ):
        api_manager.visit_steps.create_visit(create_visit_request)
        api_manager.visit_steps.create_raw_visit(
            create_visit_request, ErrorKeys.ERROR_KEY, ErrorMessages.VISIT_OVERLAPS
        )

    @pytest.mark.check_visit_persisted(
        expected_count=2, request_source=("create_visit_request_with_stop_time", "create_visit_request")
    )
    @allure.title("Create Visit With Another Ended Visit")
    def test_create_visit_with_another_ended_visit(
        self, api_manager: ApiManager, create_visit_request: CreateVisitRequest,
