...
class TestCreatePatientWithNewPerson:
    ...
    def test_create_patient_with_new_person_admin(self, api_manager: ApiManager):
        ...
        api_manager.database_steps.verify_patient_created_with_new_person(
            patient_uuid=patient.uuid, identifiers=request.identifiers
        )
    ...
    def test_create_patient_with_new_person_allowed_roles(self, api_manager, create_user_with_roles, role):
        user_request, _ = create_user_with_roles()

        request = api_manager.user_steps.build_create_patient_request()

        patient = api_manager.user_steps.create_patient(patient_request=request, user_request=user_request)
    ...
    def test_create_patient_with_new_person_without_required_privileges(self, api_manager, create_user_with_privileges):
    ...
    @pytest.mark.check_all_patients_change(delta=0, should_exist=False)
    def test_create_patient_with_new_person_disabled_user(self, api_manager, create_user_with_roles):
        ...
        api_manager.user_steps.create_patient_with_new_person_invalid_request(
            ...
            request_returns_bad_request_with_message("Privileges required: Get Identifier Types")
        )

        api_manager.database_steps.verify_patient_not_created_by_identifier(original_identifier)
    ...
    def test_create_patient_missing_person(self, api_manager):
        ...
        api_manager.user_steps.create_patient_with_new_person_invalid_request(
            patient_request=request, response_spec=ResponseSpecs.request_returns_bad_request_with_message("person")
        )
    ...
    def test_create_patient_rollback_on_invalid_identifier(self, api_manager):
        ...
        api_manager.user_steps.create_patient_with_new_person_invalid_request(
            patient_request=request,
            response_spec=ResponseSpecs.request_returns_bad_request_with_message(ErrorMessages.INVALID_SUBMISSION)
        )
        ...
        api_manager.database_steps.verify_person_not_created_by_identity(
            given_name=original_given_name, family_name=original_family_name, birthdate=original_birthdate
        )
    ...
    def test_create_patient_with_new_person_invalid_identifier(self, api_manager, field, value, error_message):
        request = api_manager.user_steps.build_create_patient_request()
